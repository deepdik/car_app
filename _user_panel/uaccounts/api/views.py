...
# -----client-----jofGCIjJDNjl2z1jkk2N0qRA2FGdOur9
...
class EmailVerificationView(APIView):
    def post(self,request,*args,**kwargs):
        ...
        if userObj:
            ...
            subject = 'Activate Your Carmension Account'
            message = render_to_string('account_activation_email.html', {
                'user': userObj,
                # 'domain':'localhost:8000',
                # 'domain':'18.188.184.119',
            ...
            email = EmailMultiAlternatives(
                        subject, plain_message, 'Carmension <webmaster@localhost>', to=[uemail]
            )
...
class UserFavoriteGarageListView(APIView):
    ...
    def get(self,request,*args,**kwargs):
        user=request.user.ruser
        garage_list=FavoriteGarage.objects.filter(user=user).values('garage')
